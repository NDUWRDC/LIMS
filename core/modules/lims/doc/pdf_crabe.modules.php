 ...
 */
class pdf_crabe extends CommonDocGenerator
{
     /**
				...
				*/
				...
				if ($conf->incoterm->enabled)
				{
					...
					if ($desc_incoterms)
					{
						...
						$pdf->Rect($this->margin_left, $tab_top - 1, $this->page_width - $this->margin_left - $this->margin_right, $height_incoterms + 1);
				...
				for ($i = 0; $i < $nblines; $i++)
				{
					...
					$pdf->MultiCell($this->page_width - $this->margin_right - $this->postotalht, 3, $total_excl_tax, 0, 'R', 0);
...
/*
	protected function _tableau_versements(&$pdf, $object, $posy, $outputlangs, $heightforfooter = 0)
	{
		...
		if ($resql)
		{
			...
			{
				$y += 3;
				if ($tab3_top + $y >= ($this->page_height - $heightforfooter))
				{
					$y = 0;
					$current_page++;
					$pdf->AddPage('', '', true);
...
/*
	protected function _tableau_info(&$pdf, $object, $posy, $outputlangs)
	{
		...
		if ($this->issuer->country_code == 'FR' && empty($mysoc->tva_assuj))
		{
				...
				foreach ($this->localtax1 as $localtax_type => $localtax_rate)
				{
					...
					foreach ($localtax_rate as $tvakey => $tvaval)
					{
						if ($tvakey != 0)    // On affiche pas taux 0
						{
							...
							$tvacompl = '';
							if (preg_match('/\ASTERISK/', $tvakey))   // Replace ASTERISK with * 
							{
