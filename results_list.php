...
/**
 *   	\file       results_list.php
 *		\ingroup    lims
 *		\brief      List page for results
 */
...
// load lims libraries
require_once __DIR__.'/class/results.class.php';
...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'resultslist'; // To manage different context of search
...
$hookmanager->initHooks(array('resultslist')); // Note that conf->hooks_modules contains array
...
$permissiontoadd = $user->rights->lims->results->write;
$permissiontodelete = $user->rights->lims->results->delete;
...
//$help_url="EN:Module_Results|FR:Module_Results_FR|ES:Módulo_Results";
$help_url = '';
$title = $langs->trans('ListOf', $langs->transnoentitiesnoconv("RElabelResults"));
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page)
{
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/lims/results_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton = dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/lims/results_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendResultsRef";
$modelmail = "results";
$objecttmp = new Results($db);
