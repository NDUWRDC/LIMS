...
/**
 *   	\file       limits_list.php
 *		\ingroup    lims
 *		\brief      List page for limits
 */
...
// load lims libraries
require_once __DIR__.'/class/limits.class.php';
...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'limitslist'; // To manage different context of search
...
$hookmanager->initHooks(array('limitslist')); // Note that conf->hooks_modules contains array
...
$permissiontoadd = $user->rights->lims->limits->write;
$permissiontodelete = $user->rights->lims->limits->delete;
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page)
{
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/lims/limits_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton = dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/lims/limits_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);
...
$topicmail = "SendLimitsRef";
$modelmail = "limits";
$objecttmp = new Limits($db);
