...
/**
 * \file        class/limits.class.php
 * \ingroup     lims
 * \brief       This file is a CRUD class file for Limits (Create/Read/Update/Delete)
 */
 ...
 * Class for Limits
 */
class Limits extends CommonObject
{
	/**
	 ...
	 */
	public $element = 'limits';

	/**
	 * @var string Name of table without prefix where object is stored. This is also the key used for extrafields management.
	 */
	public $table_element = 'lims_limits';
	...
	/**
	 * @var string String with name of icon for limits. Must be the part after the 'object_' into object_limits.png
	 */
	public $picto = 'limits@lims';
	 ...
	 */
	 public $fields=array(
		...
		'label' => array('type'=>'varchar(255)', 'label'=>'LIlabelLabel', 'enabled'=>1, 'position'=>30, 'notnull'=>1, 'visible'=>1, 'searchall'=>1, 'css'=>'minwidth200', 'help'=>"LIlabelLabelHelp", 'showoncombobox'=>'1',),
		'description' => array('type'=>'text', 'label'=>'LIlabelDescription', 'enabled'=>1, 'position'=>60, 'notnull'=>0, 'visible'=>3, 'help'=>"LIlabelDescriptionHelp",),
		'note_public' => array('type'=>'html', 'label'=>'NotePublic', 'enabled'=>1, 'position'=>61, 'notnull'=>0, 'visible'=>0, 'help'=>"LIlabelNotePublicHelp",),
		'note_private' => array('type'=>'html', 'label'=>'NotePrivate', 'enabled'=>1, 'position'=>62, 'notnull'=>0, 'visible'=>0, 'help'=>"LIlabelNotePrivateHelp",),
		...
		'status' => array('type'=>'smallint', 'label'=>'Status', 'enabled'=>1, 'position'=>1005, 'notnull'=>1, 'visible'=>4, 'default'=>'0', 'index'=>1, 'arrayofkeyval'=>array('0'=>'Draft', '1'=>'Validated', '9'=>'Canceled'),),
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->rights->lims->limits->read) {
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/*if (! ((empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->lims->limits->write))
		 || (! empty($conf->global->MAIN_USE_ADVANCED_PERMS) && ! empty($user->rights->lims->limits->limits_advance->validate))))
		 {
		...
		if (!empty($num)) {
			...
			{
				// Call trigger
				$result = $this->call_trigger('LIMITS_VALIDATE', $user);
				if ($result < 0) $error++;
				// End call triggers
			}
		}
		...
		if (!$error)
		{
			...
			if (preg_match('/^[\(]?PROV/i', $this->ref))
			{
				...
				$sql = 'UPDATE '.MAIN_DB_PREFIX."ecm_files set filename = CONCAT('".$this->db->escape($this->newref)."', SUBSTR(filename, ".(strlen($this->ref) + 1).")), filepath = 'limits/".$this->db->escape($this->newref)."'";
				$sql .= " WHERE filename LIKE '".$this->db->escape($this->ref)."%' AND filepath = 'limits/".$this->db->escape($this->ref)."' and entity = ".$conf->entity;
				$resql = $this->db->query($sql);
				...
				$dirsource = $conf->lims->dir_output.'/limits/'.$oldref;
				$dirdest = $conf->lims->dir_output.'/limits/'.$newref;
				if (!$error && file_exists($dirsource))
				{
					...
					if (@rename($dirsource, $dirdest))
					{
						...
						$listoffiles = dol_dir_list($conf->lims->dir_output.'/limits/'.$newref, 'files', 1, '^'.preg_quote($oldref, '/'));
	...
	public function setDraft($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_DRAFT, $notrigger, 'LIMITS_UNVALIDATE');
	}
	...
	public function cancel($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_CANCELED, $notrigger, 'LIMITS_CLOSE');
	}
	...
	public function reopen($user, $notrigger = 0)
	{
		...
		return $this->setStatusCommon($user, self::STATUS_VALIDATED, $notrigger, 'LIMITS_REOPEN');
	}
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1, $alt_label = '')
	{
		...
		$url = dol_buildpath('/lims/limits_card.php', 1).'?id='.$this->id;
		...
		$hookmanager->initHooks(array('limitsdao'));
	...
	public function getNextNumRef()
	{
		...
		if (empty($conf->global->LIMS_LIMITS_ADDON)) {
			$conf->global->LIMS_LIMITS_ADDON = 'mod_limits_standard';
		}
		...
		if (!empty($conf->global->LIMS_LIMITS_ADDON))
		{
			...
			$file = $conf->global->LIMS_LIMITS_ADDON.".php";
			$classname = $conf->global->LIMS_LIMITS_ADDON;
	...
	public function generateDocument($modele, $outputlangs, $hidedetails = 0, $hidedesc = 0, $hideref = 0, $moreparams = null)
	{
		...
		if (!dol_strlen($modele)) {
			$modele = 'standard_limits';

			if ($this->model_pdf) {
				$modele = $this->model_pdf;
			} elseif (!empty($conf->global->LIMITS_ADDON_PDF)) {
				$modele = $conf->global->LIMITS_ADDON_PDF;
			}
		}
