...
/**
 *   	\file       limits_card.php
 *		\ingroup    lims
 *		\brief      Page to create/edit/view limits
 */
...
$hookmanager->initHooks(array('limitscard', 'globalcard')); // Note that conf->hooks_modules contains array
...
$permissiontodelete = $user->rights->lims->limits->delete || ($permissiontoadd && isset($object->status) && $object->status == $object::STATUS_DRAFT);
$permissionnote = $user->rights->lims->limits->write; // Used by the include of actions_setnotes.inc.php
$permissiondellink = $user->rights->lims->limits->write; // Used by the include of actions_dellink.inc.php
...
if (empty($reshook))
{
	...
	$backurlforlist = dol_buildpath('/lims/limits_list.php', 1);

	if (empty($backtopage) || ($cancel && empty($id))) {
		if (empty($backtopage) || ($cancel && strpos($backtopage, '__ID__'))) {
			if (empty($id) && (($action != 'add' && $action != 'create') || $cancel)) $backtopage = $backurlforlist;
			else $backtopage = dol_buildpath('/lims/limits_card.php', 1).'?id='.($id > 0 ? $id : '__ID__');
		}
	}

	$triggermodname = 'LIMS_LIMITS_MODIFY'; // Name of trigger action code to execute when we modify record
...
if ($action == 'create')
{
	print load_fiche_titre($langs->trans("NewObject", $langs->transnoentitiesnoconv("Limits")), '', 'object_'.$object->picto);
...
{
	print load_fiche_titre($langs->trans("Limits"), '', 'object_'.$object->picto);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create')))
{
	...
	$head = limitsPrepareHead($object);
	print dol_get_fiche_head($head, 'card', $langs->trans("Limits"), -1, $object->picto);
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="'.dol_buildpath('/lims/limits_list.php', 1).'?restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToList").'</a>';
	...
	$morehtmlref.=$form->editfieldkey("LIlabelLabel", 'label', $object->label, $object, $user->rights->lims->limits->write, 'string', '', 0, 1);
	$morehtmlref.=$form->editfieldval("LIlabelLabel", 'label', $object->label, $object, $user->rights->lims->limits->write, 'string', '', null, null, '', 1);
	...
	if ($action != 'presend' && $action != 'editline') {
		...
		if (empty($reshook))
		{
			...
			// Clone
			if ($permissiontoadd) {
				print '<a class="butAction" href="'.$_SERVER['PHP_SELF'].'?id='.$object->id.'&socid='.$object->socid.'&action=clone&object=limits">'.$langs->trans("ToClone").'</a>'."\n";
			}
	...
	if ($action != 'presend')
	{
		...
		if ($includedocgeneration) {
			...
			$genallowed = $user->rights->lims->limits->read; // If you can read, you can build the PDF to read content
			$delallowed = $user->rights->lims->limits->write; // If you can create/edit, you can remove a file on card
			print $formfile->showdocuments('lims:Limits', $object->element.'/'.$objref, $filedir, $urlsource, $genallowed, $delallowed, $object->model_pdf, 1, 0, 0, 28, 0, '', '', '', $langs->defaultlang);
		}

		// Show links to link elements
		$linktoelem = $form->showLinkToObjectBlock($object, null, array('limits'));
		...
		$morehtmlright = '<a href="'.dol_buildpath('/lims/limits_agenda.php', 1).'?id='.$object->id.'">';
