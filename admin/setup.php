...
/**
 * \file    lims/admin/setup.php
 * \ingroup lims
 * \brief   LIMS setup page.
 */
...
require_once '../lib/lims.lib.php';
//require_once "../class/myclass.class.php";

// Translations
$langs->loadLangs(array("admin", "lims@lims"));
...
} elseif ($action == 'specimen')
{
	...
	{
		$file = dol_buildpath($reldir."core/modules/lims/doc/pdf_".$modele."_".strtolower($tmpobjectkey).".modules.php", 0);
		if (file_exists($file))
		{
			$filefound = 1;
...
} elseif ($action == 'setmod')
{
	...
	$tmpobjectkey = GETPOST('object');
	$constforval = 'LIMS_'.strtoupper($tmpobjectkey)."_ADDON";
...
print load_fiche_titre($langs->trans($page_name), $linkback, 'object_lims@lims');

// Configuration header
$head = limsAdminPrepareHead();
dol_fiche_head($head, 'settings', '', -1, "lims@lims");

// Setup page goes here
echo '<span class="opacitymedium">'.$langs->trans("LIMSSetupPage").'</span><br><br>';
					...
					while (($file = readdir($handle)) !== false)
					{
						if (strpos($file, 'mod_'.strtolower($myTmpObjectKey).'_') === 0 && substr($file, dol_strlen($file) - 3, 3) == 'php')
						{
							...
							if ($module->isEnabled())
							{
								...
								$constforvar = 'LIMS_'.strtoupper($myTmpObjectKey).'_ADDON';
