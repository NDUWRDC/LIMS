...
/**
 *   	\file       methods_list.php
 *		\ingroup    lims
 *		\brief      List page for methods
 */
...
// load lims libraries
require_once __DIR__.'/class/methods.class.php';
...
$contextpage = GETPOST('contextpage', 'aZ') ? GETPOST('contextpage', 'aZ') : 'methodslist'; // To manage different context of search
...
$hookmanager->initHooks(array('methodslist')); // Note that conf->hooks_modules contains array
...
$permissiontoadd = $user->rights->lims->methods->write;
$permissiontodelete = $user->rights->lims->methods->delete;
...
if ($num == 1 && !empty($conf->global->MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE) && $search_all && !$page)
{
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/lims/methods_card.php', 1).'?id='.$id);
	exit;
}
...
$newcardbutton = dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/lims/methods_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);
...
$topicmail = "SendMethodsRef";
$modelmail = "methods";
$objecttmp = new Methods($db);
