...
/**
 *   	\file       equipment_card.php
 *		\ingroup    lims
 *		\brief      Page to create/edit/view equipment
 */
...
dol_include_once('/lims/class/equipment.class.php');
dol_include_once('/lims/lib/lims_equipment.lib.php');
...
$hookmanager->initHooks(array('equipmentcard', 'globalcard')); // Note that conf->hooks_modules contains array
...
$permissiontoread = $user->rights->lims->equipment->read;
$permissiontoadd = $user->rights->lims->equipment->write; // Used by the include of actions_addupdatedelete.inc.php and actions_lineupdown.inc.php
$permissiontodelete = $user->rights->lims->equipment->delete || ($permissiontoadd && isset($object->status) && $object->status == $object::STATUS_DRAFT);
$permissionnote = $user->rights->lims->equipment->write; // Used by the include of actions_setnotes.inc.php
...
if (empty($reshook))
{
	...
	$backurlforlist = dol_buildpath('/lims/equipment_list.php', 1);
	...
	{
		$object->setValueFrom('fk_soc', GETPOST('fk_soc', 'int'), '', '', 'date', '', $user, 'EQUIPMENT_MODIFY');
	}
	...
	$triggersendname = 'EQUIPMENT_SENTBYMAIL';
...
if ($action == 'create')
{
	print load_fiche_titre($langs->trans("NewObject", $langs->transnoentitiesnoconv("Equipment")), '', 'object_'.$object->picto);
...
{
	print load_fiche_titre($langs->trans("Equipment"), '', 'object_'.$object->picto);
...
if ($object->id > 0 && (empty($action) || ($action != 'edit' && $action != 'create')))
{
	...
	dol_fiche_head($head, 'card', $langs->trans("Equipment"), -1, $object->picto);
	...
	if ($action == 'delete')
	{
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"].'?id='.$object->id, $langs->trans('DeleteEquipment'), $langs->trans('ConfirmDeleteObject'), 'confirm_delete', '', 0, 1);
	}
	...
	// Object card
	// ------------------------------------------------------------
	$linkback = '<a href="'.dol_buildpath('/lims/equipment_list.php', 1).'?restore_lastsearch_values=1'.(!empty($socid) ? '&socid='.$socid : '').'">'.$langs->trans("BackToList").'</a>';
	...
	if ($action != 'presend')
	{
		...
		$linktoelem = $form->showLinkToObjectBlock($object, null, array('equipment'));
		...
		$morehtmlright = '<a href="'.dol_buildpath('/lims/equipment_agenda.php', 1).'?id='.$object->id.'">';
